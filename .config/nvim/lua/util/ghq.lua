-- ghq + snacks.picker リポジトリランチャー
--
-- もともと shell 関数 `g` (ghq + fzf) で行っていたリポジトリ選択を、
-- nvim 内のフロート (snacks.picker) として開けるよう書き直したもの。
-- 各リポジトリの状態 (dirty / ahead / behind / branch) を一覧に表示し、
-- プレビューに README (bat) とコミット履歴を出す。
-- 状態取得は元と同様 xargs -P で並列化し、snacks の proc finder に
...
-- 色付けは picker 側 (format) で行うため、ここではプレーンな値のみ出す。
...
-- finder の 1 行 (タブ区切り) を picker アイテムのフィールドに展開する。
-- パースできない行は false を返してスキップする。
local function transform(item)
  local n, a, b, name, branch, path =
    item.text:match("^(.-)\t(.-)\t(.-)\t(.-)\t(.-)\t(.+)$")
  if not path then
    return false
  end
  ...
  item.branch = branch
  item.dir = path
  item.dirty = tonumber(n) or 0
  item.ahead = tonumber(a) or -1
  item.behind = tonumber(b) or -1
  -- 絞り込み対象 (リポジトリ名 + ブランチ + パス) を text に入れる。
  item.text = name .. " " .. branch .. " " .. path
end

-- 1 エントリを色付きで整形する。色はテーマ追従の標準 hl グループを使う。
-- カラム幅は dirty / ahead / behind / name の順。
local function format(item)
  ...
  if item.dirty > 0 then
    ret[#ret + 1] = { align("*" .. item.dirty, 4), "DiagnosticError" }
  else
    ret[#ret + 1] = { align(".", 4), "Comment" }
  end
  ret[#ret + 1] = { " " }
  ...
  if item.ahead < 0 then
    ...
    if item.ahead > 0 then
      ret[#ret + 1] = { align("↑" .. item.ahead, 4), "DiagnosticOk" }
    else
      ret[#ret + 1] = { align("↑0", 4), "Comment" }
    end
    ret[#ret + 1] = { " " }
    if item.behind > 0 then
      ret[#ret + 1] = { align("↓" .. item.behind, 4), "DiagnosticWarn" }
    else
      ret[#ret + 1] = { align("↓0", 4), "Comment" }
    end
  end
  ...
  -- リポジトリ名
  ret[#ret + 1] = { align(item.name, 30), "Identifier" }
  ret[#ret + 1] = { " " }
...
-- カスタムプレビュー: README + コミット履歴を固定レイアウトで描画する。
local function preview(ctx)
  ...
  local out = vim.fn.systemlist({
    "zsh", "-c", PREVIEW_SCRIPT, "_", item.dir, tostring(half), tostring(height),
  })
  ...
  -- README を markdown として treesitter 着色する。
  ...
  for i, l in ipairs(lines) do
    if l:match("^↑") then
      pcall(vim.api.nvim_buf_add_highlight, buf, ns, "DiagnosticOk", i - 1, 0, -1)
    elseif l:match("^────") then
      pcall(vim.api.nvim_buf_add_highlight, buf, ns, "Title", i - 1, 0, -1)
    elseif l:match("^  %x+ ") then
      pcall(vim.api.nvim_buf_add_highlight, buf, ns, "Comment", i - 1, 0, 9)
    end
  end
  return true
end
...
function M.pick(opts)
  Snacks.picker.pick(vim.tbl_deep_extend("force", {
    ...
    -- 検索入力ではなく j/k で選択できるよう list に focus して起動する (検索は i / 入力で開始)。
    focus = "list",
