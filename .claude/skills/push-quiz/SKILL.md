...
3. **実装報告の承認 (ship の report フェーズ、`ship.sh report approve` 済み) が出題の前提**。未承認なら先に報告を提示してユーザー承認を得る。報告なしでいきなり AskUserQuestion を出すことは禁止
4. 答えを問題文や選択肢の並びで示唆しない。もっともらしい誤答選択肢を混ぜる。**正解の位置は問題ごとに変える** (LLM は正解を先頭に置きがち。作問後に「正解が同じ位置に並んでいないか」を確認し、偏っていたら並べ替えてから出題する)
...
`ship.sh report status` で承認済み SHA が現 HEAD と一致することを確認する。未承認・不一致なら、差分の実物に基づく実装報告 (変更内容・設計判断・検証結果) をテキストで提示し、ユーザー承認後に `ship.sh report approve` を実行してから進む。報告とクイズを同一メッセージに混ぜない (ユーザーが報告を読む時間を確保する)。
