...
3. **出題前に必ず実装報告を行う**。ユーザーは報告を読んでからクイズに臨む。報告なしでいきなり AskUserQuestion を出すことは禁止
...
クイズを出す前に、差分の実物に基づく実装報告をテキストで提示する。内容:

- 何をどう変更したか (変更点の要約と主要なファイル / 箇所)
- なぜそう実装したか (設計判断・採用した方式と捨てた代替案)
- 動作確認・検証の結果

報告を出し切ってから次のステップに進む。報告とクイズを同一メッセージに混ぜない (ユーザーが報告を読む時間を確保する)。
