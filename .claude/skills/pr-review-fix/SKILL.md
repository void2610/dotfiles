...
- 依存: `gh` CLI (authenticated、v2.40+ 推奨)、`git`。`jq` は不要 (`gh api --jq` の内蔵 gojq を使用)。
...
Phase 6 でコミットするたび `commit_hash` を、Phase 7 で返信・resolve するたび `replied` / `resolved` を更新する。`outputs/` はリポジトリの `.gitignore` に含まれるのが通例なので commit されない。
