...
const GIT_SKILL_TARGET_RE = /\b(git\s+commit|git\s+push|gh\s+pr\s+create)\b/;
const GIT_SKILL_MARKER_RE =
...
export const register: Register = (on) => {
  on("tool.call", { tool: "Bash" }, async ($, e, next) => {
    ...
    if (GIT_SKILL_TARGET_RE.test(cmd) && !GIT_SKILL_MARKER_RE.test(cmd)) {
      return {
        deny: "git commit / git push / gh pr create の直接実行はブロックされました。commit / pr-create / pr-review-fix スキル経由で (CLAUDE_GIT_SKILL=<skill名> を前置して) 実行してください。",
      };
    }

    if (!BRANCH_OP_RE.test(cmd) || CHECKOUT_PATHS_RE.test(cmd)) return next(e);

    const inRepo =
      (await $.process.run(["git", "rev-parse", "--is-inside-work-tree"]))
        .exitCode === 0;
    if (!inRepo) return next(e);

    // (1) ship フローのロック。exit 3 のみがロック検知 (guard 自体の失敗で誤ブロックしない)
    const home = await $.env.get("HOME");
    if (home) {
      const guard = await $.process.run([
        "bash",
      ...
      if (guard.exitCode === 3) {
        return {
          deny: `ブランチ操作をブロック: ${(guard.stdout + guard.stderr).trim()}`,
        };
      }
    }

    if (USER_ALLOW_RE.test(cmd)) return next(e);
    ...
    if (!current || !defaultBranch || current === defaultBranch) return next(e);
    ...
    if (!dirty) {
      ...
      if (remoteRef) {
        const count = await $.process.run([
          "git",
          "rev-list",
          "--count",
          `${remoteRef}..HEAD`,
        ]);
        if (count.exitCode === 0 && count.stdout.trim() === "0") return next(e);
      }
    }

    return {
      deny: `ブランチ操作をブロック: 現在ブランチ '${current}' には OPEN な PR があり、未コミットまたは未 push の変更が残っています。commit / push で提出を完了させてから移動すること。それでも移動が必要ならユーザーに報告し指示を待ち、明示承認があった場合のみ 'SHIP_ALLOW_BRANCH_SWITCH=1 <cmd>' で再実行すること。`,
    };
  });
};
