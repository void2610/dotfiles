...
def _pick_fresher(a, b):
    ...
    return (
        a if (a.get("used_percentage") or 0) >= (b.get("used_percentage") or 0) else b
    )
...
# manual-mode-ask-all.sh hook が書き出す permission mode を読み、Manual 中はバッジ表示
try:
    _sid = data.get("session_id", "")
    _pmode_path = os.path.join(os.environ.get("TMPDIR", "/tmp"), f"claude-pmode-{_sid}")
    with open(_pmode_path) as _f:
        if _f.read().strip() == "default":
            parts.insert(0, "\033[1;38;2;255;100;100mMANUAL\033[0m")
except Exception:
    pass
...
if current_dir:
    ...
    if toplevel:
        ...
        path_part = (
            repo_name
            if rel_path == "."
            else repo_name + "/" + rel_path.replace(os.sep, "/")
        )
